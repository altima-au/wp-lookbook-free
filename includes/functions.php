 ...
 * @return string
 */
function lb_form_select($select_name, $value, $options_list=array(), $attrbutes = array()) {
    $select_attr = '';
    if (count($attrbutes)) {
 ...
 * @return array
 */
function check_slider_image_restriction($file, $restrictions) {
...
function cleen_fx(&$el_ar, $removed = array('all','none')) {
    if(in_array($el_ar, $removed)){
        unset($el_ar);
    }
}

function woocommerce_not_install_notice() {
    echo
